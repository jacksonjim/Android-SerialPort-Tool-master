...
public class SerialPortManager {
     ...
     * @return
     */
    public SerialPort open(Device device, boolean showHex) {
        return open(device.getPath(), device.getBaudrate(), showHex);
    }
     ...
     * @return
     */
    public SerialPort open(String devicePath, String baudrateString, boolean showHex) {
        if (mSerialPort != null) {
            close();
        }
    ...
    public void sendCommand(final String command, boolean isHex) {
        ...
        SendMessage sendMessage = new SendMessage(command);
        byte[] bytes;
        if (isHex) {
            bytes = ByteUtil.hexStr2bytes(command);
        }else {
            bytes = command.getBytes();
        }
        sendCommand(bytes, sendMessage);
    }

    /**
     * 发送命令包
     */
    public void sendCommand(final byte[] bytes, final SendMessage sendMessage) {
        ...
        rxSendData(bytes).subscribeOn(mSendScheduler).subscribe(new Observer<Object>() {
            ...
            public void onNext(Object o) {
                LogManager.instance().post(sendMessage);
            }
